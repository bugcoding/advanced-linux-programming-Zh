...
通常运行程序的方式就是，在shell命令窗口输入程序的名字并回车。而根据不同使用情况，还可以在输入程序名称之后加上一个或者多个以空格分开的单词作为程序的附加信息，而这个附加信息，就叫命令行参数（参数中如果带空格，可以使用转义字符转义）。一般情况，这被称为程序的参数列表，但它（参数）不是必须在shell命令行获取的。在第三章“进程”中，你可看到另一种调用程序的方式，就是程序可以直接指定另一个程序做为参数列表。

程序在shell中被调用的时候，参数列表已经包含了整个输入命令行输入的内容，其中包括了程序的名称和所有在命令行已经提供的参数。打个比方，     可以像下面所示在shell窗口中调用ls命令显示根目录下的内容和对应文件的大小：

> % ls –s / 

这个ls程序的参数列表接收了三个元素，第一个就是在命令行里指定的叫ls程序自己的名字，第二个和第三个参数列表元素就是二个命令行参数 –s和/。

程序的main函数可以通过其参数argc和argv访问参数列表（如果你不使用[二个参数]，就可以忽略它们），第一个参数argc是存储参数列表参数数量的一个整型，第二个参数argv，是一个字符指针数组，数组的大小就是argc，数组元素就是指向参数列表的每一个以NULL结束的字符串参数。
...
几乎所有的GNU/Linux程序都遵从关于命令行参数如何解释执行的一些约定。程序参数可以分为二类：选项（或者叫做标志）和其他参数。当其他参数为程序提供输入（如实例数据，输入文件等）的时候，选项参数就会改变程序的行为。
...
通常一个程序所支持的选项都会提供短和长二种形式，前者是后者的简化。例如，一般的程序都可以解释-h和—help选项，而且它们的作用完全相同。正常情况下，在shell中调用程序，可以直接在程序名称后接任意多个选项，而有些选项只希望后接一个参数。比如许多程序，详细解释选项 –output foo的意思就是，程序输出至一个名为foo的文件内。很多选项后可能还会跟着一些输入文件或者输入数据等一些命令行参数。

例如，ls –s /命令是显示根目录的内容，-s选项改变了ls的默认行为，而指示其显示每个文件条目的大小（kb为单位）。而/参数告诉ls应该列出哪个目录的内容。--size与-s选项是相同的作用，所以还可以像ls –size /这样调用而达到同样儿的目的。

GNU 编码标准列出了一些常用命令行选项的名字，如果你也计划提供一些像这样的选项，使用编码标准里指定的名字是一个好想法，如你想要你的程序表现得更像其他那些程序并且更易于用户使用，你可以在大多数Linux系统命令行里调用如下命令来查阅一下关于命令行选项的GNU 编码标准指南：
...
标准C库提供了标准输入输出流（分别是stdin和stdout），这些都会被scanf，printf还有其他函数用到。在传统的UNIX中，使用标准输入输出是Linux程序的习惯。它允许多个一系列的程序通过shell管道重定向输入和输出（可以查看你shell的帮助手册来了解它的语法）。
...
当调用一个程序的时候，有的时候需要同时将标准输出和标准错误重定向到一个文件或者管道。这个语法可以适用在各种shell中，对于Borune类的shell（包括bash，大多数Linux发行版默认的shell），语法形式是：

> % program >output_file.txt?>&1
>
> % program 2 > &1 | filter        

这个2>&1的语法表示，文件描述符2(stderr)被合并到文件描述符1(标准输出)中，需要注意的是，这个2>&1必须在文件重定向之后（如第一个例子）或者必须在一个管道重定向之前。

应该明白stdout是缓冲的，写入stdout的数据直到缓冲区满，程序关闭或stdout关闭之后才会发送到控制台（如果是重定向，也可能是其他设备），你可以像下边这样儿调用函数强制刷新缓冲区：

```c
fflush(stdout);
```

正好相反，stderr是不缓冲的，写入stderr的数据会直达控制台。[1][1]

这会产生一些令人惊讶的结果。例如，这个循环不会每秒打印一个句点，相反，句点是缓冲的，当缓冲满了的时候会打印出一串句点：
...
当一个程序退出的时候，它会用一个退出码来指明它的退出状态。这个退出码是一个小的整型值。按照惯例，退出码为0 意味着程序成功运行，一个非0值表明程序发生了错误。一些程序用不同非0值的退出码来区别特殊的错误。

在大多数的shell中，可以使用一个特殊的变量$?获取最近一次运行的程序的退出值。这有一个调用二次ls命令在每次调用后打印退出值的例子，第一种情况ls正确执行并且返回了退出码0，第二种情况ls命令遇到了一个错误（因为命令行里指定的文件名不存在），于是就返回了一个非0的退出码。
...
[1]: 在C++中，区别依次是cout和cerr。注意的是endl刷新流并打印一个换行符，如果你不想刷新流（比如出于性能的原因），使用常量字符’\n’代替。


一个C或者C++程序退出码，是由main函数的返回值来指定的。还有其他方式在程序非正常情况下结束时给程序提供和指定退出码（使用信号量）。这将会在第三章讨论。
...
你的shell，像其他程序一样有一个环境。Shell提供了一种直接查看和修改环境变量的方法。调用printenv程序在shell中打印当前环境变量。各种shell都有不同的的内置语法来使用环境变量。下面的是针对于Bourne风格shell的语法。
...
你可以使用export命令把一个shell变量导出到环境变量中，比如，设置EDITOR环境变量，可以使用下面的命令：
...
在程序中，可以使用<stdlib.h>里的getenv函数获取环境变量。函数接受一个变量名称参数，并以字符串返回相应的环境变量值，如果在环境中没定义对应的变量就返回NULL。如要设置或者清除环境变量，依次使用setenv和unsetenv函数。

列举出所有的环境变量有一些难度，要这么做的话，必须访问一个定义在GNU C库里特殊的名为environ的全局变量。这个变量类型是char**，是一个指向以NULL结束的字符串数组的指针。每个字符串包含了一个形式为VARIABLE=VALUE的环境变量。
...
不要修改你自己的environ变量，用setenv和unsetenv函数替代。

通常，一个新的程序开始运行，它其实是继承了这个调用它的程序环境的副本（如果在交互环境下调用，就是shell）。所以，举例说，运行在shell里的程序可能会检查你在shell中设置的环境变量。

环境变量通常被用来与程序传递配置信息。举个例子，你正在写一个连接互联网服务器并获取一些信息的程序。你写的程序，应该方便的在命令行中指定服务器的名字。但是，假设服务器名字不是用户经常去更改的东西。你可以使用一个特殊的环境变量——叫SERVER_NAME——指定服务器名字，如果那个环境变量不存在，就会使用一个默认值。你程序的一部应该会像清单2.4所示：        
...
假设程序名字叫client，假定你没有设置SERVER_NAME变量，默认服务器名就会被使用：
...
> % exportSERVER_NAME=backup-server.elsewhere.net
>
> % client
>
...
有时候程序需要生成一个临时文件暂时存储一些大的数据，或者用于给另外一个程序传递数据。在GNU/Linux系统上，临时文件存储在/tmp目录下。使用临时文件的时候，你要注意以下几个陷阱：

1)    你的程序可能同时有多个实例在运行(被相同的或者不同的用户)。这些实例应该使用不同的临时文件名，防止它们产生冲突。

2)    临时文件应该设置权限，以防止未被授权的用户通过修改或者替换临时文件的方式更改程序的运行。

3)    程序的生成方式应该不被外部所能预测，也就是说，攻击者可以利用在（程序）测试一个文件名是否在使用和打开一个新文件之间的延迟。
...
mkstemp函数从文件名模板里创建一个唯一的临时文件名，创建的这个临时文件权限仅使当前用户能访问和打开文件读写。文件名模板是一个以XXXXXX（六个大写X）结尾的字符串；mkstemp函数替换用字符替换X以便文件名是唯一的。返回值是一个文件描述符；使用write家族的函数来对临时文件进行写操作。

mkstemp函数生成的文件不会自动删除，临时文件不再需要的时候需要您手动去删除它（程序员应该仔细的清理临时文件；不然，/tmp文件系统最后会满，系统会无法运行）。如果临时文件只是在内部使用而不是对另一个程序处理，马上调用unlink是个好办法。unlink函数会删除对应文件的目录入口，但是由于文件系统中文件引用计数的关系，如果不再打开那个文件的文件描述符，这个临时文件就不会被移除，这样，你的程序可以继续使用这个临时文件，而且会在你关闭文件描述符的时候文件自动消失。因为Linux在程序结果的时候会关闭文件描述符，所以即使你的程序非正常结束，临时文件也会被移除。

这对函数在mkstemp示例的清单2.5中，一起使用，这些函数将使写内存缓冲到临时文件，之后再读取回来的操作变得非常简单。
...
如果你的程序对一个空指针解除引用操作，会提供比错误信息更有用的信息
...
根据你的程序和系统调用的性质，对应的动作一旦失败就会打印出一条错误信息，取消操作，中止程序，重试，甚至忽略错误。想方设法包含处理所有失败情况逻辑在代码中，这是非常重要的。
...
这个错误信息说明尽管libtest.a已经包含了f的定义，但链接器仍然找不到它。那是因为在首次遇到libtest.a的时候它就被搜索了，在那个时候，链接器没有看到任何关于f的引用【注，意为在链接的时候如果找到未处理的引用，那么链接器会向后查找，而此时，libtest已被搜索过了，再向后找不到对应的引用，就会报错】。
