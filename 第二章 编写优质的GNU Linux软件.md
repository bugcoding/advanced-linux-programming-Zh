...
当一个程序退出的时候，它会用一个退出码来指明它的退出状态。这个退出码是一个小的整型值。按照惯例，退出码为0 意味着程序成功运行，一个非0值表明程序发生了错误。一些程序用不同非0值的退出码来区别出各种错误。
...
1)    程序可能同时有多个实例在运行(被相同的或者不同的用户)。这些实例应该使用不同的临时文件名，防止它们产生冲突。
