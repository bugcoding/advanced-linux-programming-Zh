...
与`/dev/random`不同的是，从`/dev/urandom`读取字节流从不会锁定。如果Linux的随机源耗尽，就会使用一个加密算法来根据旧的随机字节序列来生成新的"伪随机数"。这样生成的随机数已经可以就能应付大多数的情况了，这些生成的随机数并不像在`/dev/random`中获得的那样通过严格的随机性测试。
