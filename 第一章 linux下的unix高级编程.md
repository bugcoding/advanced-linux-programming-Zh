...
这章将会教你如何以必要的基本步骤创建一个Linux下的c/c++程序，另外，这章还会教你怎样去创建和修改和编译c/c++源代码，并且最后调试程序结果。如果你已对于Linux的编程非常熟练，那么你可以跳过这里直接看章节2（写标准的GNU/Linux软件），应该多多留意一下2.3节，“编写和使用库”，对于静态和动态链接你可能知道得并不是很清楚。

针对于这本书，我们假定你熟悉c/c++语言和C标准库的大多数函数。这本书除了解释C++语言的特性和复杂性的时候，基他示例源代码均是用C来写的，我们也假定你了解最基本的Linux Shell命令，比如创建目录或者是复制文件之类的。考虑到大多数的Linux 程序员最初的编程环境是在windows上，我们也会偶尔的指出Windows和Linux开发环境的相似和不同之处。
...
一旦使用-g编译，编译器就会在生成的目标文件和执行文件的时候包含额外的信息，调试器使用这些信息就能指示出源文件哪一行对应的哪个地址，打印出本地变量等等。
...
这个命令在main[6][6]函数的第一行设置了一个断点，现在试着重新像下面这样带参数运行程序：
...
帮助手册包含了大量非常有用的信息，寻求帮助的时候应该在第一时间想到它，命令帮助手册中记录了命令的描述，命令行选项和参数，输入输出，错误代码，配置，等等这些，系统调用的帮助手册记录了库函数描述，函数参数，返回值，错误代码列表及副作用，并且指定了调用这个函数需要包含哪个文件等。
